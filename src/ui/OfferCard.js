...
import { deriveBadgeFromDeal, deriveEstimatedValueLabel, sanitizeHtml, splitDescription } from "../utils/format";
    ...
    <div className="border border-white p-[4px] bg-black">
      <div className="border border-white p-4 flex flex-col gap-4 h-full">
        ...
        <div className="flex flex-col gap-2 tw-offer-text">
          ...
          <div className="tw-offer-desc text-white/80 text-[0.95rem] leading-[1.35]" dangerouslySetInnerHTML={{ __html: sanitizeHtml(descriptionHtml) }} />
        </div>
...
function PriceBlock({ offer, badge, value }) {
  ...
  const showValue = !!value.value && dealType !== "special_offer" && dealType !== "special-offer";

  if (dealType === "discount" && (offer.discount_type || "").toLowerCase() === "percentage") {
    return (
      <div className="flex flex-col gap-1">
        <div className="tw-discount flex items-end flex-wrap">
          <span className="text-white text-[2rem] font-[700] uppercase leading-none tracking-1">{offer.discount ?? 0}</span>
          <span className="text-white text-[2rem] font-[700] uppercase leading-none">%</span>
          <span className="text-white text-[2rem] font-[700] uppercase leading-none">&nbsp;OFF</span>
          {showValue ? (
            <div className="flex items-end gap-2 ml-2 leading-none">
              <span className="text-[#898989] text-[1.375rem] uppercase tracking-wide">{value.label}</span>
              <span className="text-[#898989] text-[2rem] font-[700] uppercase">{value.value}</span>
            </div>
          ) : null}
        </div>
      </div>
  ...
  if (dealType === "special_offer" || dealType === "special-offer") {
    return (
      <div className="flex flex-col gap-1">
        <div className="text-white text-[1.5rem] font-[700] uppercase">Special Offer</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="tw-discount flex items-end gap-2 flex-wrap">
        <span className="text-white text-[2rem] font-[700] uppercase leading-none">{badge.primary}</span>
        {badge.secondary ? (
          <span className="text-white text-[2rem] font-[700] uppercase leading-none">{badge.secondary}</span>
        ) : null}
        {showValue ? (
          <div className="flex items-end gap-2 ml-2 leading-none">
            <span className="text-[#898989] text-[1.375rem] uppercase tracking-wide">{value.label}</span>
